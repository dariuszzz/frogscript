...
impl<'a> ARM64Backend<'a> {
    ...
    pub fn offset_calc_asm(
        ...
        let offset = match &addr.offset {
            IRAddressOffset::DataPageOffset(_) => unimplemented!(),
        ...
        match &addr.addr {
            ...
            IRAddressType::DataPage(_) => unimplemented!(),
    ...
    pub fn compile_ir(&mut self, symbol_table: &SymbolTable) -> Result<String, String> {
        ...
        // x0-29 (no x18) and d0-30
        let mut legal_registers = general_regs;
        legal_registers.append(&mut float_regs);

        let mut regalloc = RegallocState::new(legal_registers);
        regalloc.allocate_registers(&mut asm_ir);

        let asm_instructions = asm_ir.pretty_print();
        ...
        write!(out, "{asm_instructions}").unwrap();
    ...
    // pub fn compile_ir_old(&mut self, symbol_table: &SymbolTable) -> Result<String, String> {
    //     self.adjust_ssa_for_target(symbol_table);
    ...
    //         if let Some(_) = block.starts_func {
